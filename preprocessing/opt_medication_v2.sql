...
SELECT 
    ...
    -- For week 2 event with GROUP_CONCAT
    GROUP_CONCAT(
        CASE 
            WHEN BINARY redcap_event_name = 'Step 1 Week 2 (Arm 2: Step 1 Acute)' AND 
                 (LOWER(adherence_3_med1) LIKE 'bupropion%' OR LOWER(adherence_3_med1) LIKE 'aripiprazole%') 
            THEN adherence_3_med1 
            ELSE NULL 
        END
        ORDER BY adherence_3_med1
        SEPARATOR ', '
    ) AS week2_med1,
    ...
    GROUP_CONCAT(
        CASE 
            WHEN BINARY redcap_event_name = 'Step 1 Week 2 (Arm 2: Step 1 Acute)' AND 
                 (LOWER(adherence_3_med2) LIKE 'bupropion%' OR LOWER(adherence_3_med2) LIKE 'aripiprazole%') 
            THEN adherence_3_med2 
            ELSE NULL 
        END
        ORDER BY adherence_3_med2
        SEPARATOR ', '
    ) AS week2_med2,
    ...
    GROUP_CONCAT(
        CASE 
            ...
            ELSE NULL 
        END
        ORDER BY adherence_3_freq2
        SEPARATOR ', '
    ) AS week2_med2_freq,

    -- For week 4 event with GROUP_CONCAT
    GROUP_CONCAT(
        CASE 
    ...
    -- For week 6 event with GROUP_CONCAT
    GROUP_CONCAT(
        CASE 
    ...
    -- For week 8 event with GROUP_CONCAT
    GROUP_CONCAT(
        CASE 
    ...
    -- For week 10 event with GROUP_CONCAT
    GROUP_CONCAT(
        CASE 
